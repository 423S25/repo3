...
2. Go to your repository at [423S25/hrdc-intranet](https://github.com/423S25/hrdc-intranet).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `intranet.thehrdc.org` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | intranet.thehrdc.org | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/423S25/hrdc-intranet/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/423S25/hrdc-intranet/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/423S25/hrdc-intranet-backend`) and Frontend (`ghcr.io/423S25/hrdc-intranet-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of intranet.thehrdc.org](https://github.com/423S25/hrdc-intranet/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/intranet.thehrdc.org.yml` stack.
