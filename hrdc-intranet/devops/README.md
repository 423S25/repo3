...
DEPLOY_ENV=prod
DEPLOY_HOST=intranet.thehrdc.org
DEPLOY_PORT=22
DEPLOY_USER=plone
DOCKER_CONFIG=.docker
STACK_NAME=intranet-thehrdc-org
```
...
You need either a Ubuntu or Debian based system for intranet.thehrdc.org, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    intranet.thehrdc.org:
      ansible_user: root
      host: intranet
      hostname: intranet.thehrdc.org
```
...
Deploy the stack defined in `devops/stacks/intranet.thehrdc.org.yml` to the remote server with:
